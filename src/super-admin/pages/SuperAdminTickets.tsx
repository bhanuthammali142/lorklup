...
export function SuperAdminTickets() {
  ...
  const load = async () => {
    setLoading(true)
    let q = supabase.from('platform_tickets').select('*, hostels(name)').order('created_at', { ascending: false })
  ...
  const updateStatus = async (id: string, status: string) => {
    const { error } = await supabase.from('platform_tickets').update({ status, updated_at: new Date().toISOString() }).eq('id', id)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center"><Loader2 className="h-7 w-7 animate-spin text-indigo-400 mx-auto" /></div>
        ) : tickets.length === 0 ? (
          <div className="p-12 text-center text-slate-400">
            <Ticket className="h-10 w-10 mx-auto mb-3 opacity-20" />
            <p className="font-medium">No tickets found. {filter !== 'all' ? 'Try changing the filter.' : ''}</p>
              ...
              <div key={ticket.id} className="p-5 hover:bg-slate-50 transition">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${STATUS_COLORS[ticket.status] || 'bg-slate-50 text-slate-600'}`}>
                      {ticket.status}
                    </span>
