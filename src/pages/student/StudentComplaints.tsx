 ...
 *
 * Changes: added Supabase Realtime subscription on complaints table so
 * when admin changes a status (pending → in-progress → resolved),
 * the student sees it update instantly without refreshing.
 */
...
export function StudentComplaints() {
  ...
  useEffect(() => {
    ...
    // Realtime: admin updates complaint status → student sees it live
    const channel = supabase
      .channel(`complaints:student:${studentData.id}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          ...
          if (newStatus === 'resolved') {
            ...
            toast('Your complaint is now being worked on.', { icon: '🔧' })
          }
        }
      )
      .subscribe()
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 sm:zoom-in-95">
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-400 text-xs mt-0.5">Describe the problem and attach a photo if helpful</p>
              </div>
              <button
